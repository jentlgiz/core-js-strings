...
function removeLastOccurrences(str, value) {
  const firstOccur = str.lastIndexOf(value);
  if (firstOccur === -1) {
    return str;
  }
  return str.slice(0, firstOccur) + str.slice(firstOccur + value.length);
...
function isPalindrome(str) {
  const unwantChar = /[\W_]/g;
  const lowRegStr = str.toLowerCase().replace(unwantChar, '');
  const palindromeStr = lowRegStr.split('').reverse().join('');
  return palindromeStr === lowRegStr;
}
